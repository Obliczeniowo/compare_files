        ...
        if os.path.exists(os.path.join(path, file1)):
            ...
            for file2 in filelist[nr:]:
                if os.path.exists(os.path.join(path, file2)):
                    if compareFile(os.path.join(path, file1), os.path.join(path, file2), 0) == 0:
            ...
            print("Ścieżka nie istnieje" + os.path.join(path, file1))
    
    delete = True
    deleted = 0
    if len(filelists):
        data = open(path + "files.txt", "w", encoding="utf-8")
        for i in filelists:
            for j in i:
                data.write(str(j) + "\n")
            if delete:
                for j in i:
                    if os.path.exists(os.path.join(path, j[1])):
                        os.remove(os.path.join(path, j[1]))
                        deleted += 1
